...
import { CheckCircle, Loader2, Upload, AlertCircle, Trash2, AlertTriangle, Info, Brain, ShieldCheck, ShieldAlert, ClipboardList, FileText, Copy, Check } from "lucide-react";
...
function formatWarningForClipboard(w: ValidationWarning): string {
  ...
  if (items.length > 0) {
    lines.push("");
    for (const d of items) lines.push(d.startsWith("•") || d.startsWith("·") ? d : `• ${d}`);
  }
  return lines.join("\n");
}
...
function CopyAllWarnings({ warnings }: { warnings: ValidationWarning[] }) {
  const [copied, setCopied] = useState(false);
              ...
              {validationWarnings.length === 0 ? (
                ...
                <>
                  {validationWarnings.length > 1 && (
                    <CopyAllWarnings warnings={validationWarnings} />
                  )}
