...
export default function UploadPergantianPage() {
  const [step, setStep] = useState<ImportStep>("idle");
  const [parsed, setParsed] = useState<ProductChangeItem[] | null>(null);
  ...
  const handleFileSelect = useCallback(async (file: File) => {
    ...
    try {
      ...
      const items = parseProductChanges(wb);
      const period = extractPeriodLabel(wb);
      setParsed(items);
      setPeriodLabel(period);
      setFileName(file.name);
      setStep("preview");
      if (items.length === 0) {
        toast.warning("File dibaca tapi tidak ada data pergantian produk.");
      } else {
        toast.success(`File dibaca: ${items.length} item pergantian produk`);
      }
    } catch {
  ...
  const runImport = async () => {
    ...
    try {
      ...
      setResult(count);
      setStep("done");
      toast.success(`Import berhasil: ${count} item`);
    } catch (err) {
  ...
  const totalValue = parsed?.reduce((s, i) => s + i.totalPrice, 0) ?? 0;
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 p-4 md:p-6 lg:p-8">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Upload Pergantian Produk</h1>
        ...
        <div className="lg:col-span-2 space-y-6 flex flex-col">
          ...
          {step === "preview" && parsed && (
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <div className="rounded-xl border border-border p-4 bg-card shadow-sm flex flex-col justify-center">
                  <span className="text-muted-foreground text-xs uppercase tracking-wider font-semibold mb-1">Total Item</span>
                  <p className="font-medium text-2xl">{parsed.length}</p>
                </div>
                <div className="rounded-xl border border-border p-4 bg-card shadow-sm flex flex-col justify-center">
                  <span className="text-muted-foreground text-xs uppercase tracking-wider font-semibold mb-1">Total Nilai</span>
                  <p className="font-bold text-lg text-primary truncate" title={formatRpFull(totalValue)}>{formatRpFull(totalValue)}</p>
                </div>
              </div>
                ...
                <div className="overflow-x-auto max-h-[500px] overflow-y-auto">
                  <table className="w-full text-xs">
                    <thead className="bg-muted/50 sticky top-0 z-10 backdrop-blur-sm">
                      <tr>
                        {["Nama Bahan", "Expired", "Unit", "Harga", "Qty", "PPN", "Total"].map((h) => (
                    ...
                    <tbody className="divide-y divide-border/50">
                      {parsed.map((item, i) => (
                        <tr key={i} className="hover:bg-muted/20 transition-colors">
                          ...
                          <td className="px-4 py-2.5 text-muted-foreground whitespace-nowrap">{item.expiredDate ?? "-"}</td>
                          <td className="px-4 py-2.5 whitespace-nowrap">{item.unit ?? "-"}</td>
                          <td className="px-4 py-2.5 text-right font-mono whitespace-nowrap">{formatRpFull(item.unitPrice)}</td>
                          <td className="px-4 py-2.5 text-right whitespace-nowrap">{item.qty}</td>
                          <td className="px-4 py-2.5 text-right font-mono text-muted-foreground whitespace-nowrap">{formatRpFull(item.ppn)}</td>
                          <td className="px-4 py-2.5 text-right font-mono text-primary font-semibold whitespace-nowrap">{formatRpFull(item.totalPrice)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          {step === "importing" && (
            <div className="rounded-xl border border-border bg-card p-12 flex flex-col items-center justify-center gap-4 shadow-sm animate-in fade-in zoom-in-95 duration-300">
              ...
              <p className="text-base font-medium">Menyimpan data pergantian produk...</p>
            </div>
          )}
          ...
          {step === "done" && result !== null && (
            <div className="rounded-xl border border-green-200 bg-green-50 dark:bg-green-950/20 dark:border-green-900 p-8 flex flex-col items-center justify-center gap-5 shadow-sm animate-in fade-in zoom-in-95 duration-500 text-center">
              ...
              <div>
                ...
                <p className="text-green-600/80 dark:text-green-400/80 font-medium">{result} item telah tersimpan ke database</p>
              </div>
              <button 
                onClick={reset} 
                  ...
                  {uploadGroups.map((g) => (
                    <div key={g.periodLabel} className="flex items-center gap-3 p-3 rounded-xl hover:bg-muted/50 transition-colors group">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground truncate" title={g.fileName}>{g.fileName} · {g.count} item</p>
                      </div>
                      <button
                        onClick={async () => {
